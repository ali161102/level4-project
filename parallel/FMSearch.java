...
public class FMSearch {
    ...
    private static ArrayList<String> create_input(int len_paths) {
        ...
        for (int i = 0; i < (Math.pow(3, len_paths)); i++) {
            // String str = Integer.toBinaryString(i);
            String str = Integer.toUnsignedString(i, 3);
            //System.out.println(str);
            while (str.length() < len_paths) {
                str = "0" + str;
            }
            paths.add(str.replace('0', 'p').replace('1', 'b').replace('2', 's'));
            // 0 = plain, 1 = bob
        ...
        for (char call : inputcalls.toCharArray()) {
            ...
            if (call == 'p') {
                ...
                if (call == 'b') {
                    ...
                    if (call == 's') {
                        // single at end of lead
                        current_lh = lead_lastrow(current_lh).substring(0, 4) + next_lh_plain.substring(4);
    ...
    public static String lead_lastrow(String lh) {
        char[] row = new char[method.length];
        for (int bell = 0; bell < method.length; bell++) {
            row[method[bell][method[0].length-2]] = lh.charAt(bell);
        }
        return(new String(row));   
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(create_input(3));
        System.out.println(lead_lastrow("12345678"));
        ...
        for (String path : input_paths) {
            ...
            if (sd != null) {
                ...
                FMRunnable sr = new FMRunnable(num_started, method, sd);
